"""Linear-head counterpart to train_matryoshka.py, added in response to a
pre-submission review: the paper's Matryoshka compression result (Table:
Accuracy by embedding dimension) was trained and tested only for the
structured (Attention+Composition) core, never for the linear head, leaving
open whether clean nested-dimension compression is a property of Matryoshka
training in general or something specific to the structured architecture.

Identical recipe to train_matryoshka.py in every respect except the
trainable module: same shared XLM-R-base backbone, same two-stage NLI
pretrain -> EN/BN/TE multilingual fine-tune, same matryoshka_info_nce_loss
(_hard_negatives), same seed, same hyperparameters. The only variable that
changes is LinearProjectionHead (mean-pool + single nn.Linear) in place of
CognitiveEmbeddingCore.

Saves to results/core_model_matryoshka_linear.pt -- does NOT touch
core_model_matryoshka.pt (the existing structured-head reference checkpoint).
...
def main():
    ...
    core = LinearProjectionHead(hidden_dim)

    # ---- Stage 0: NLI pretrain (Matryoshka loss) ----
    print("=== Stage 0: NLI pretrain (Matryoshka, linear head) ===")
    ...
    print("\n=== Stage 1: multilingual joint fine-tune (Matryoshka, linear head) ===")
    optimizer = torch.optim.Adam(core.parameters(), lr=LR)
    ...
    torch.save(best_state, RESULTS_DIR / "core_model_matryoshka_linear.pt")
    print(f"\nTotal time: {time.time() - t_start:.1f}s")
    print(f"Best macro val (768-dim): {best_macro:.4f}")
    print(f"Saved to {RESULTS_DIR / 'core_model_matryoshka_linear.pt'}")
