...
\begin{abstract}
Sentence-embedding models often add cognitively-motivated pooling mechanisms without testing them against the simplest alternative under one controlled protocol. This paper is that controlled attribution study: a small trainable head combining attention pooling and BiGRU-based composition, evaluated against an identically-trained linear projection on a frozen encoder, across five languages (English, Bangla, Telugu, Hindi, Arabic) and three task types. At the fixed, untuned capacity used throughout ($7.5\times$ more parameters, $2.7$--$2.8\times$ slower training for the structured head), the linear head has the higher point estimate in twenty-one of twenty-two comparisons -- a descriptive count, not an independent-trials statistic, since checkpoints are reused across correlated comparisons. Statistically supported differences hold for only a subset: Bangla and Hindi significantly favor the linear head, English significantly favors the structured head, and Telugu and Arabic are inconclusive. The attribution goes further than a single verdict: a parameter-matched control (four of five languages, same leak-free protocol) shows capacity mismatch reverses Hindi's result outright, is too weak to treat as established in Arabic, narrows Bangla's gap substantially without reversing it, and does not narrow Telugu's gap at all. Capacity is therefore a substantial but language-dependent confound in this comparison, not a global explanation for the negative result. The negative headline result is therefore a property of this specific, fixed, untuned configuration, not a general verdict on structured or cognitively-motivated pooling. Memory-aware retrieval, predictive modeling, Matryoshka compression, and cross-lingual transfer are also tested directly as secondary properties, rather than assumed. The fair-baseline comparison and capacity-matched control together provide a reusable protocol for evaluating any pooling mechanism before publication.
\end{abstract}
